  ...
  connect() {
    ...
    async function initMap() {
      ...
      properties.forEach((e) => {
        ...
        priceTag.className =
          "inline-flex items-center px-2 py-1 text-sm font-medium text-white bg-red-500 rounded-md ring-1 ring-inset ring-green-600/20";

        priceTag.textContent = e.dataset.price;
        const marker = new AdvancedMarkerElement({
          map: map,
          position: {
            lat: parseFloat(e.dataset.latitude),
            lng: parseFloat(e.dataset.longitude),
          },
          content: priceTag,
          // title: 'haha'
        });
